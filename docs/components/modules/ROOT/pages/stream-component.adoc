...
| *retry* (consumer) | Will retry opening the stream if it's overwritten, somewhat like tail --retry <p/> If reading from files then you should also enable the fileWatcher option, to make it work reliable. | false | boolean
