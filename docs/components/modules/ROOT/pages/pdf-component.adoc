...
| *textProcessingFactory* (producer) | Text processing to use. <ul> <li>autoFormatting: Text is getting sliced by words, then max amount of words that fits in the line will be written into pdf document. With this strategy all words that doesn't fit in the line will be moved to the new line. <li>lineTermination: Builds set of classes for line-termination writing strategy. Text getting sliced by line termination symbol and then it will be written regardless it fits in the line or not. There are 2 enums and the value can be one of: autoFormatting, lineTermination | lineTermination | TextProcessingFactory
|===
