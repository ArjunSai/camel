...
| *startingSha* (consumer) | The starting sha to use for polling commits with the commit consumer. The value can either be a sha for the sha to start from, or use <tt>beginning to start from the beginning, or <tt>last to start from the last commit. | last | String
